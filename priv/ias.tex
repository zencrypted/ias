...
\begin{itemize}
  ...
  \item \textbf{Implemented:} майстер прив’язаного до пристрою налаштування VPN з перевірками переходів, готовності матеріалів, виправленням помилок та відновленням чернетки.
  ...
  \item \textbf{Implemented:} звіряння стану, виявлення осиротілих записів, відновлення або виведення з експлуатації та відновлення після перезапуску.
  \item \textbf{Partial:} CA enrollment; поточний CMP-шлях через зовнішній OpenSSL CLI є перехідним, а OpenSSL має бути повністю усунений із IAS runtime.
...
\subsection{IAS і CA}
\begin{itemize}
  \item \textbf{AS IS / Partial:} IAS містить CMP enrollment adapter, який запускає зовнішній OpenSSL CLI і звертається до CA CMP endpoint.
  \item \textbf{Target:} IAS не запускає OpenSSL або інші зовнішні криптографічні CLI. CSR та контекст випуску передаються до CA через керований нативний клієнт або сервісний адаптер; CMP може бути збережений як протокол взаємодії.
...
\begin{itemize}
  ...
  \item retry, restart recovery і звіряння стану після недоступності VPN.
\end{itemize}
...
\begin{itemize}
  ...
  \item Осиротілий запис, невідповідність fingerprint, застаріла ревізія та неавторизований стан VPN повинні відображатися як інциденти звіряння стану.
...
\begin{itemize}
  \item IAS повинен аудувати lifecycle transitions, issuance/import, authorization decisions, доставку конфігурацій, інциденти звіряння стану та дії відновлення.
...
IAS-FR-003 & CA enrollment без зовнішнього OpenSSL CLI у IAS runtime & Target & керований нативний клієнт або CA adapter \\
IAS-FR-004 & Майстер прив’язаного до пристрою налаштування VPN & Implemented & wizard, readiness, relationships, authorization \\
IAS-FR-005 & IAS $\rightarrow$ VPN RPC delivery & Implemented & VPN client/delivery modules and CT suites \\
IAS-FR-006 & Revision, digest, idempotence і stale protection & Implemented & стан доставки та підготовки конфігурацій, тести \\
IAS-FR-007 & Звіряння стану, відновлення осиротілих записів і виведення з експлуатації & Implemented & reconciliation/recovery modules and CT suites \\
...
\begin{enumerate}
  ...
  \item Повністю усунути запуск зовнішнього OpenSSL CLI з IAS runtime, зберігши CMP або інший погоджений контракт через керований CA client/adapter.
  \item Прогнати і стабілізувати повний життєвий цикл майстра налаштування VPN.
