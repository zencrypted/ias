...
wizard_provisions_device_into_vpn_runtime(Config) ->
    ...
    {ok, InitialRuntimePeer} = rpc:call(VpnNode,
                                         ...
                                         [client_a],
                                         ?RPC_TIMEOUT_MS),
    InitialRevision = maps:get(revision, InitialRuntimePeer),
    ...
    ?assertEqual(InitialRevision + 1, command_revision(DeliveryResult)),
    ...
    ?assertEqual(InitialRevision + 1, maps:get(revision, InitialDelivery)),

    {ok, RevokeResult} = ias_vpn_access_lifecycle:revoke(DeviceId),
    ?assertEqual(client_a, maps:get(runtime_peer_id, RevokeResult)),
    ?assertEqual(revoke, maps:get(operation, RevokeResult)),
    ?assertEqual(InitialRevision + 2, maps:get(revision, RevokeResult)),
    ?assertEqual(applied, maps:get(delivery_status, RevokeResult)),
    {ok, RevokedDeliveryPeer} = maps:get(runtime, RevokeResult),
    ?assertEqual(client_a, maps:get(id, RevokedDeliveryPeer)),
    ...
    ?assertEqual(InitialRevision + 2, maps:get(revision, RevokedRuntimePeer)),
    ...
    ?assertNot(lists:member(client_a, running_peers(VpnNode))),

    {ok, RejectedEnableResult} = ias_vpn_access_lifecycle:enable(DeviceId),
    ?assertEqual(client_a, maps:get(runtime_peer_id, RejectedEnableResult)),
    ?assertEqual(enable, maps:get(operation, RejectedEnableResult)),
    ?assertEqual(InitialRevision + 3, maps:get(revision, RejectedEnableResult)),
    ...
    ?assertEqual(InitialRevision + 3,
                 maps:get(current_revision, LifecycleStatus)),
