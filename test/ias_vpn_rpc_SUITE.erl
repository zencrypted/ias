...
wizard_provisions_device_into_vpn_runtime(Config) ->
    ...
    ?assertEqual(false, maps:is_key(certificate_fingerprint, CommandDesired)),
    ?assert(FixtureFingerprint =/= DynamicFingerprint),
    ?assertEqual(1, maps:get(revision, RuntimePeer)),
    ?assertEqual(ias, maps:get(provisioning_source, RuntimePeer)),
    ?assertEqual(upsert, maps:get(last_provisioning_operation, RuntimePeer)),
    ...
    ?assertEqual(1, maps:get(revision, GatewayPeer)),
    ?assertEqual(ias, maps:get(provisioning_source, GatewayPeer)),
    ?assertEqual(upsert, maps:get(last_provisioning_operation, GatewayPeer)),
